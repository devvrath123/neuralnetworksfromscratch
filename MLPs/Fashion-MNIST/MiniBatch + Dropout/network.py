...
def train(network, error, error_prime, X_train, Y_train, iter = 100, lr = 0.001, batch_size = 64, decay = 0.01, details = True):
    ...
    for j in range(iter):
        ...
        if details and (j + 1) % 5 == 0:
            print(f"Iteration {j+1}/{iter}, lr = {current_lr:.6f}, error = {iter_error:.6f}")
